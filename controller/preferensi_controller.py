...
def get_user_preferences(db: Session, user_id: int):
    """Mengambil semua preferensi bobot milik seorang user."""
    return db.query(bobot_model.BobotPreferensi).filter(bobot_model.BobotPreferensi.id_pengguna == user_id).all()

def set_user_preferences(db: Session, user_id: int, preferences_data: BobotCreate):
    ...
    new_preferences = []
    # Masukkan semua preferensi baru dari request
